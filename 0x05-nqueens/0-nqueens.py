#!/usr/bin/python3
""" NQuee task  NxN chessboard ##solved """
...
def is_valid_queen_placement(queens_positions: list) -> bool:
    ...
    """
    n_r = len(queens_positions) - 1
    for row in range(n_r):
        column_difference = abs(queens_positions[row] - queens_positions[n_r])
        if column_difference == 0 or column_difference == n_r - row:
            return False
    return True
